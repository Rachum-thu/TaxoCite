...
method, namely Scalable transformers for weather forecasting (Stormer), on WeatherBench 2 [ 41],
...
forecasts at a small interval “Xωt = fω (X0), in which ωt is typically from 6 to 24 hours. To produce
...
function [22, 26, 5, 7]. In iterative forecasting, one can forecast either the weather conditions Xωt or
the weather dynamics !ωt = Xωt − X0, and Xωt can be recovered by adding the predicted dynamics
...
!ωt = Xωt − X0, which is the difference between two consecutive weather conditions, X0 and
Xωt, across the time interval ωt. A common practice in previous works [ 22, 26] is to use a small
fixed value of ωt such as 6 hours. However, as we show in Figure 3a, while small intervals tend to
...
L(ω) = Eωt∼P (ωt),(X0 ,Xωt)∼D || fω (X0, ωt) − !ωt ||2_2,
...
∑_{i=1}^H
∑_{j=1}^W
w(v)L(i)(!^v_{ij,ωt} − !^v_{ij,ωt})^2 ].

The expectation is over ωt, X0, and Xωt which we omit for notational simplicity. In this equation,
...
In the second and third phases, we finetune the trained model from the preceding phase with K =
...
finetuning was used in FourCastNet [35] and also adopted in more recent works [22, 26].
...
final forecast by averaging the individual predictions. This approach achieves a similar effect to
...
offers two primary advantages. First, it reduces the sequence length by a factor of V, significantly
...
present the complete implementation details of the weather-specific embedding in Section B.
Figure 3b shows the superior performance of weather-specific embedding to standard patch embedding
at all lead times from 1 to 10 days. A similar weather-specific embedding module was introduced by
ClimaX [32] to improve the model’s flexibility when handling diverse data sources with heterogeneous
...
Following weather-specific embedding, the tokens are processed by a stack of transformer blocks [ 45].
...
architecture. We refer to [32] for illustrations of the weather-specific embedding block.
...
five atmospheric variables – Geopotential (Z), Temperature (T), U and V components of wind (U and
...
Dynamics vs. absolute forecasts: We justify our decision to forecast the dynamics !ωt by comparing
with a counterpart that forecasts Xωt. Figure 5c shows that forecasting the changes in weather
...
GraphCast at long lead times. However, FuXi requires finetuning multiple models specialized for
...
followed by Graph-EFM [ 33], which combined a hierarchical variant of Graphcast with the VAE
